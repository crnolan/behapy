...
def tdt2bids(
    session_fn: str, experiment_fn: str, bids_root: str, sourcedata_root: str
) -> None:
    ...
    convert_block(session_df, Path(bids_root), ep["event_names"], ep["invert_events"])


def tdt2bids_command():
    logging.basicConfig(format="%(levelname)s:%(message)s", level=logging.INFO)
    parser = argparse.ArgumentParser(description="Convert TDT tanks into BIDS format")
    parser.add_argument(
        "session_fn", type=str, help="path to CSV file with TDT session information"
    )
    parser.add_argument(
        "experiment_fn", type=str, help="path to JSON file with experiment details"
    )
    parser.add_argument(
        "bids_root",
        type=str,
...
def medpc2csv(source_pattern: str, output_path: str, config_fn: str) -> None:
    ...
    config["event_map"] = {
        int(key): value for key, value in config["event_map"].items()
    }
    for fn in glob.glob(source_pattern):
        variables = medpc.parse_file(fn)
        info = medpc.experiment_info(variables)
        events = medpc.get_events(
            variables[config["timestamp"]],
...
def medpc2csv_command():
    ...
    parser.add_argument(
        "source_pattern", type=str, help="path pattern for source files"
    )
    parser.add_argument(
        "output_path", type=str, help="folder for the two output CSV files"
    )
    parser.add_argument(
        "config_fn", type=str, help="config file defining events variables"
    )
    args = parser.parse_args()
    medpc2csv(**vars(args))
...
def preprocess_dash(bidsroot):
    ...
    signals = recordings.loc[
        :, ["subject", "session", "task", "run", "label"]
    ].drop_duplicates()

    def get_recording(index):
        r = signals.iloc[index]
        signal = fp.load_signals(bidsroot, r.subject, r.session, r.task, r.run, r.label)
        return signal
...
def preprocess_dash_command():
    logging.basicConfig(format="%(levelname)s:%(message)s", level=logging.INFO)
    parser = argparse.ArgumentParser(description="Load the preprocessing dashboard")
    parser.add_argument("bidsroot", type=str, help="path to the BIDS root")
    args = parser.parse_args()
...
def preprocess(bidsroot):
    ...
    def save_recording(row):
        try:
            fp.preprocess(
                bidsroot, row.subject, row.session, row.task, row.run, row.label
            )
...
def preprocess_command():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Preprocess the dataset from raw data using the rejected " "intervals"
        )
    )
    parser.add_argument("bidsroot", type=str, help="path to the BIDS root")
    args = parser.parse_args()
