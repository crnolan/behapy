...
def vis_raw(df):
    """Plot fibre photometry trace (non-interactively)."""
...
def getAutoShifts(df):
    bounds = None
    # TODO: has the baseline shifted? if yes, we *might* want to fit the
    # isosbestic trace in two separate regressions
    return bounds
...
def reject_overlay(intervals, selected=[]):
    if intervals is None or intervals == []:
        return hv.Overlay([])
    colors = [
        'red' if x in selected else 'pink' for x in range(
            len(intervals))]
...
def vis_raw_interactive(df, detectBads, detectShifts):
    """Plot fibre photometry trace interactively for artefact rejection."""
    ...
    reject_stream = streams.BoundsX(
        source=trace_shade,
        boundsx=bounds,
        transient=True)  # plot those data segments

    select_stream = streams.DoubleTap(
        source=trace_shade, x=None, y=None, transient=True)
    reject_dmap = hv.DynamicMap(
        record_reject, streams=[
            reject_stream, select_stream])

    # combine trace and dynamic map into one figure, and return it
    return (trace_shade * reject_dmap).opts(height=300, responsive=True)
